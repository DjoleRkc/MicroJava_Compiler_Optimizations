// generated with ast extension for cup
// version 0.8
// 29/6/2026 19:40:29


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Mulop Mulop) { }
    public void visit(MethodDecl MethodDecl) { }
    public void visit(InterfaceDecl InterfaceDecl) { }
    public void visit(Literal Literal) { }
    public void visit(MultiplicativeSequenceList MultiplicativeSequenceList) { }
    public void visit(Relop Relop) { }
    public void visit(Assignop Assignop) { }
    public void visit(CondTermList CondTermList) { }
    public void visit(InterfaceMethodsList InterfaceMethodsList) { }
    public void visit(FormalParametersList FormalParametersList) { }
    public void visit(OptionalRelop OptionalRelop) { }
    public void visit(ClassMethodDeclList ClassMethodDeclList) { }
    public void visit(ActualParamsList ActualParamsList) { }
    public void visit(StatementList StatementList) { }
    public void visit(ClassVarDecl ClassVarDecl) { }
    public void visit(Addop Addop) { }
    public void visit(AdditionalStatement AdditionalStatement) { }
    public void visit(ExtendsClause ExtendsClause) { }
    public void visit(Factor Factor) { }
    public void visit(OptionalFormalParameters OptionalFormalParameters) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(LocalVariablesList LocalVariablesList) { }
    public void visit(OptionalConditionList OptionalConditionList) { }
    public void visit(CondFactList CondFactList) { }
    public void visit(GlobalVarDecl GlobalVarDecl) { }
    public void visit(DeclList DeclList) { }
    public void visit(ClassVarDeclList ClassVarDeclList) { }
    public void visit(ConstDeclList ConstDeclList) { }
    public void visit(ArraySpecifier ArraySpecifier) { }
    public void visit(AccessorList AccessorList) { }
    public void visit(ActualParams ActualParams) { }
    public void visit(OptionalClassMethodsList OptionalClassMethodsList) { }
    public void visit(OperationList OperationList) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(Expr Expr) { }
    public void visit(MethodTypeName MethodTypeName) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(Decl Decl) { }
    public void visit(OptionalWidth OptionalWidth) { }
    public void visit(Statement Statement) { }
    public void visit(Type Type) { }
    public void visit(ClassDecl ClassDecl) { }
    public void visit(ConstDecl ConstDecl) { }
    public void visit(CondFact CondFact) { }
    public void visit(Setop Setop) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(DesignatorParamsList DesignatorParamsList) { }
    public void visit(ReturnValue ReturnValue) { }
    public void visit(OptionalElse OptionalElse) { }
    public void visit(BoolLiteral BoolLiteral) { visit(); }
    public void visit(CharLiteral CharLiteral) { visit(); }
    public void visit(NumberLiteral NumberLiteral) { visit(); }
    public void visit(SetType SetType) { visit(); }
    public void visit(TypeOf TypeOf) { visit(); }
    public void visit(InterfaceMethodSignatureList InterfaceMethodSignatureList) { visit(); }
    public void visit(InterfaceMethodDeclaration InterfaceMethodDeclaration) { visit(); }
    public void visit(NoInterfaceMethods NoInterfaceMethods) { visit(); }
    public void visit(InterfaceMethodSignature InterfaceMethodSignature) { visit(); }
    public void visit(InterfaceDeclaration InterfaceDeclaration) { visit(); }
    public void visit(InterfaceName InterfaceName) { visit(); }
    public void visit(ClassMethodsList ClassMethodsList) { visit(); }
    public void visit(NoClassMethods NoClassMethods) { visit(); }
    public void visit(ClassMethodDeclarationList ClassMethodDeclarationList) { visit(); }
    public void visit(NoClassMethodDeclarations NoClassMethodDeclarations) { visit(); }
    public void visit(Extends Extends) { visit(); }
    public void visit(NoExtends NoExtends) { visit(); }
    public void visit(ClassVariablesList ClassVariablesList) { visit(); }
    public void visit(NoClassVariables NoClassVariables) { visit(); }
    public void visit(ClassVarDeclarationErrorSemi ClassVarDeclarationErrorSemi) { visit(); }
    public void visit(ClassVarDeclaration ClassVarDeclaration) { visit(); }
    public void visit(ClassName ClassName) { visit(); }
    public void visit(ClassDeclarationErrorLBrace ClassDeclarationErrorLBrace) { visit(); }
    public void visit(ClassDeclaration ClassDeclaration) { visit(); }
    public void visit(IsArray IsArray) { visit(); }
    public void visit(NoArray NoArray) { visit(); }
    public void visit(GlobalVarDeclarationErrorComma GlobalVarDeclarationErrorComma) { visit(); }
    public void visit(GlobalVarDeclarationErrorSemi GlobalVarDeclarationErrorSemi) { visit(); }
    public void visit(GlobalVarDeclaration GlobalVarDeclaration) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(NoVarDeclarations NoVarDeclarations) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(FormalParamsList FormalParamsList) { visit(); }
    public void visit(NoFormalParametersList NoFormalParametersList) { visit(); }
    public void visit(FormalParameter FormalParameter) { visit(); }
    public void visit(FormalParameters FormalParameters) { visit(); }
    public void visit(NoFormalParams NoFormalParams) { visit(); }
    public void visit(VoidMethod VoidMethod) { visit(); }
    public void visit(TypeMethod TypeMethod) { visit(); }
    public void visit(MethodSignature MethodSignature) { visit(); }
    public void visit(LocalVariable LocalVariable) { visit(); }
    public void visit(NoLocalVariablesList NoLocalVariablesList) { visit(); }
    public void visit(Mod Mod) { visit(); }
    public void visit(Div Div) { visit(); }
    public void visit(Mul Mul) { visit(); }
    public void visit(Minus Minus) { visit(); }
    public void visit(Plus Plus) { visit(); }
    public void visit(SetOperator SetOperator) { visit(); }
    public void visit(DesignatorParam DesignatorParam) { visit(); }
    public void visit(NoDesignatorParamsList NoDesignatorParamsList) { visit(); }
    public void visit(ParenthesisExpression ParenthesisExpression) { visit(); }
    public void visit(NewWithParams NewWithParams) { visit(); }
    public void visit(NewWithBrackets NewWithBrackets) { visit(); }
    public void visit(Bool Bool) { visit(); }
    public void visit(Char Char) { visit(); }
    public void visit(Num Num) { visit(); }
    public void visit(FuncCall FuncCall) { visit(); }
    public void visit(MultiplicativeOperations MultiplicativeOperations) { visit(); }
    public void visit(NoMultiplicativeSequenceList NoMultiplicativeSequenceList) { visit(); }
    public void visit(Term Term) { visit(); }
    public void visit(AddOperations AddOperations) { visit(); }
    public void visit(NoOperations NoOperations) { visit(); }
    public void visit(MapExpression MapExpression) { visit(); }
    public void visit(TermOperation TermOperation) { visit(); }
    public void visit(NegativeTermOperation NegativeTermOperation) { visit(); }
    public void visit(AssignOperator AssignOperator) { visit(); }
    public void visit(ActPar ActPar) { visit(); }
    public void visit(ActualParameter ActualParameter) { visit(); }
    public void visit(NoActualParam NoActualParam) { visit(); }
    public void visit(ActualParametersList ActualParametersList) { visit(); }
    public void visit(NoActualParams NoActualParams) { visit(); }
    public void visit(BracketAccessor BracketAccessor) { visit(); }
    public void visit(DotAccessor DotAccessor) { visit(); }
    public void visit(NoAccessorList NoAccessorList) { visit(); }
    public void visit(DesignatorName DesignatorName) { visit(); }
    public void visit(Designator Designator) { visit(); }
    public void visit(SetOperation SetOperation) { visit(); }
    public void visit(Decrement Decrement) { visit(); }
    public void visit(Increment Increment) { visit(); }
    public void visit(DesignatorWithParams DesignatorWithParams) { visit(); }
    public void visit(AssignOperation AssignOperation) { visit(); }
    public void visit(Else Else) { visit(); }
    public void visit(HasElse HasElse) { visit(); }
    public void visit(NoElse NoElse) { visit(); }
    public void visit(ConditionTermList ConditionTermList) { visit(); }
    public void visit(NoCondTermList NoCondTermList) { visit(); }
    public void visit(LessOrEqual LessOrEqual) { visit(); }
    public void visit(Less Less) { visit(); }
    public void visit(GreaterOrEqual GreaterOrEqual) { visit(); }
    public void visit(Greater Greater) { visit(); }
    public void visit(NotEqual NotEqual) { visit(); }
    public void visit(Equal Equal) { visit(); }
    public void visit(OptionalRelOperator OptionalRelOperator) { visit(); }
    public void visit(NoOptionalRelop NoOptionalRelop) { visit(); }
    public void visit(CondFactor CondFactor) { visit(); }
    public void visit(ConditionFactorsList ConditionFactorsList) { visit(); }
    public void visit(NoCondFactors NoCondFactors) { visit(); }
    public void visit(ConditionTerm ConditionTerm) { visit(); }
    public void visit(Condition Condition) { visit(); }
    public void visit(ReturnVal ReturnVal) { visit(); }
    public void visit(NoReturnValue NoReturnValue) { visit(); }
    public void visit(Width Width) { visit(); }
    public void visit(NoOptionalWidth NoOptionalWidth) { visit(); }
    public void visit(AdditionalStatements AdditionalStatements) { visit(); }
    public void visit(NoAdditionalStatement NoAdditionalStatement) { visit(); }
    public void visit(OptionalCondition OptionalCondition) { visit(); }
    public void visit(NoOptionalConditions NoOptionalConditions) { visit(); }
    public void visit(While While) { visit(); }
    public void visit(DoStart DoStart) { visit(); }
    public void visit(IfStatementErrorRParen IfStatementErrorRParen) { visit(); }
    public void visit(NestedStatements NestedStatements) { visit(); }
    public void visit(DoWhile DoWhile) { visit(); }
    public void visit(Print Print) { visit(); }
    public void visit(Read Read) { visit(); }
    public void visit(Return Return) { visit(); }
    public void visit(Continue Continue) { visit(); }
    public void visit(Break Break) { visit(); }
    public void visit(IfStatement IfStatement) { visit(); }
    public void visit(DesignatorStmt DesignatorStmt) { visit(); }
    public void visit(StatementsList StatementsList) { visit(); }
    public void visit(NoStatements NoStatements) { visit(); }
    public void visit(MethodDeclaration MethodDeclaration) { visit(); }
    public void visit(MethodDeclarationList MethodDeclarationList) { visit(); }
    public void visit(NoMethodDeclarations NoMethodDeclarations) { visit(); }
    public void visit(ConstDeclarationList ConstDeclarationList) { visit(); }
    public void visit(NoConstDeclarations NoConstDeclarations) { visit(); }
    public void visit(ConstDeclarationErrorSemi ConstDeclarationErrorSemi) { visit(); }
    public void visit(ConstDeclaration ConstDeclaration) { visit(); }
    public void visit(DeclarationList DeclarationList) { visit(); }
    public void visit(NoDeclarations NoDeclarations) { visit(); }
    public void visit(InterfaceDeclarations InterfaceDeclarations) { visit(); }
    public void visit(ClassDeclarations ClassDeclarations) { visit(); }
    public void visit(GLobarVarDeclarations GLobarVarDeclarations) { visit(); }
    public void visit(ConstDeclarations ConstDeclarations) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
